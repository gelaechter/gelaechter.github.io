...
| Warning                                                                                                                                                                                                                                       | Screenshot                                |
| --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------- |
| **High entropy (Meta Defender)** <br/> The executable contains sections with high entropy. This usually suggests encryption to make reverse-engineering / understanding it harder.                                                             | ![alt text](high_entropy_warning.png)     |
| **Obfuscated Files or Information (VirusTotal)** <br/>The executable contains cryptographic-primitives like RC4 or AES. This would again suggest encryption of our binary.                                                                     | ![alt text](crypt_primitives_warning.png) |
| **Virtual Machine detection (VirusTotal)** <br/>The executable seems to contain strings related to the detection of virtual machines. This is usually done to detect the usage of Sandboxes. This way malware doesn't trigger during analysis. | ![alt text](vm_detection_warning.png)     |
| **Installation of a Google Updater? (VirusTotal)** <br/>The executable seems to drop files inside the Google Updater directory. This suggests that malware is installed.                                                                       | ![alt text](files_dropped_warning.png)    |
...
![alt text](windows_executable_entropy.png)

This graph shows the entropy of our **Linux executable**:
![alt text](linux_executable_entropy.png)
...
![alt text](pattern_data_view.png)
...
|                                                                                                                                     |                                                                                                                                            |
| ----------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------ |
| ![alt text](memory_map_view.png) <br/> The memory map view. It shows us the executable sections and their respective address ranges. | ![alt text](ghidra_executable_sections.png) The image section headers of our Windows executable. We can see the actual and assigned names. |
...
![alt text](sans_debug_entropy.png)
...
|                                                                                                                                                                                                                                     |                                                                                                                                                                                                                                                 |
| ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| ![alt text](virustotal_sans_debug.png) [The VirusTotal scan without debug symbols](https://www.virustotal.com/gui/file/247486e1367f9e59141e8eeb530ac1df4e549c0c79a2628473098397e2c13736) now shows just 6/71 instead of 9/71 flags. | ![alt text](metadefender_sans_debug.png) [The MetaDefender Cloud scan without debug symbols](https://metadefender.com/results/hash/247486E1367F9E59141E8EEB530AC1DF4E549C0C79A2628473098397E2C13736) now shows just 1/22 instead of 2/22 flags. |
...
> ![alt text](virustotal_sans_debug_updated.png)
>
...
```go { linenos=inline, linenostart=195}
//go:nosplit
...
>
>```go { linenos=inline, linenostart=574}
>//go:nosplit
>//go:cgo_unsafe_args
...
>
>```nasm { linenos=inline, linenostart=503}
...
```go { linenos=inline, linenostart=153}
...
```go { linenos=inline, linenostart=1229, hl_lines=[12]}
// AX: data
// BX: hash seed
// CX: length
...
![alt text](ghidra_aeshashbody.png)
...
![alt text](ghidra_debug_diff.png)
...
|                                                           |                                                                           |
| --------------------------------------------------------- | ------------------------------------------------------------------------- |
| ![alt text](capa.jpg) The capa logo under defense evasion | ![alt text](cape_sandbox.jpg) The CAPE Sandbox logo under defense evasion |
...
```go { linenos=inline, linenostart=6629}
...
```go { linenos=inline, linenostart=6622, hl_lines=[4]}
...
```go { linenos=inline, linenostart=6615, hl_lines=[4]}
// set sets P id's bit.
...
```go { linenos=inline, linenostart=6640, hl_lines=[11,13]}
func pidleput(pp *p, now int64) int64 {
    assertLockHeld(&sched.lock)
...
```go { linenos=inline, linenostart=6615, hl_lines=[3,10]}
// set sets P id's bit.
func (p pMask) set(id int32) {
  word := id / 32
  mask := uint32(1) << (id % 32)
...
![alt text](division_by_32.drawio.png)
...
```go { linenos=inline, linenostart=6640, hl_lines=[17] }
func pidleput(pp *p, now int64) int64 {
  assertLockHeld(&sched.lock)
...
```go { linenos=inline, linenostart=409, hl_lines=[2] }
...
```go { linenos=inline, linenostart=390, hl_lines=[2] }
...
```go { linenos=inline, linenostart=350, hl_lines=[6]}
const (
...
```go { linenos=inline, linenostart=409, hl_lines=[5]}
func (e *limiterEvent) start(typ limiterEventType, now int64) bool {
  if limiterEventStamp(e.stamp.Load()).typ() != limiterEventNone {
    return false
  }
...
```go { linenos=inline, linenostart=371, hl_lines=[2]}
...
```go { linenos=inline, linenostart=409 }
func (s limiterEventStamp) typ() limiterEventType {
  return limiterEventType(s >> (64 - limiterEventBits))
}
```
