...
Typst is a typesetter like TeX / LaTeX that went open source back in March 2023. I remember reading about it
...
This got me hooked in an instant. And so I decided to convert an old paper from LaTeX to learn about Typst.
...
Here are things I liked about Typst and the reasons why I think Typst is the future.
...
And when using online collaborative editing and sufficiently large or complex documents this can stop being
...
and splitting documents into multiple files which can then be excluded for compilation.

But when using Typst *you don't have to*! If your document compiles (Typst requires your document to be correct)
it does so **FAST**.

## Typst uses Markdown like markup

Typst markup uses a lot of Ideas from markup languages like Markdown.
This makes writing Typst a lot faster since the syntax tends to be a lot less verbose and more human-readable.

### Headings:

If I want to write headings of different depths then we use a prefix-style heading markup.
...
If we compare that to the LaTeX header macros the Typst variant is a lot easier:
...
But the Typst macros *are more explicit*,
in that when we use `\part` we are explicitly splitting our document into parts.
Whereas every heading in Typst is a "section" and by default referred to as such.

### bold, italic and monospaced text:

Bold, italic and monospaced also heavily borrows from Markdown.
Unlike Markdown italic and bold markers (`_` and `*`) are not used interchangeably.
...
From a historical standpoint it probably makes sense to have explicit names
for these text modifiers that tell us what they do.
Though in the age of context aware editors, we don't have to think
actively think about their meaning since our editors
can display the text as formatted, allowing us to see their meaning at a glance.

And now that we don't need these explicit names anymore, they often just register as noise when reading.
Compare the readability of text we would nest some of these modifiers:

```latex
\texttt{Hello}, I am a \textit{sentence that uses \textbf{nested} macros}.
```

That is way less of a problem with Typst:
...
LaTeX is the total opposite, it doesn't even support clickable links until you use the `hyperref` package.
...
Three backticks and the language to start it and three backticks to end it, pretty straight forward:
...
LaTeX in comparison again requires the use of a package,
and the use of an environment which starts with a `\begin` and ends with an `\end`.
...
In this case a single environment takes only little effort and makes for still very legible source code.
When comparing the List syntax though, this can get out of hand quickly:
...
When compared to LaTeX then Typst feels similar.
...
- *float* to position figures and similar where they actually are in the LaTeX source.
...
An important, side effect that this has is that you don't need to decide on which solution to choose
when you need some functionality. Especially since LaTeX has such a long history there are a bunch of packages,
often superseding another. So you end up in situations like:

*I need to create a table, do I use*
...
Let's image you want to note a fraction such as this:

$$
\frac{1}{2}
$$

LaTeX makes no assumptions. If you want a fraction you have to *tell it that* using the `\frac` macro.
...
the math syntax makes the slash an explicit choice by escaping it (`1\/2`).
...
And these simplifications get more weight the more we try to note:
$$
\begin{pmatrix}
1 & 2 & \ldots & 10 \\
2 & 2 & \ldots & 10 \\
...
But don't worry, if you don't have time to get used to new mathematical notaions,
just [keep using LaTeX math in Typst](https://typst.app/universe/package/mitex), with all the other benifits that Typst brings you.
...
And considering that a sizeable chunk of LaTeX users are
computer scientists i believe this to be of interest for many others.

> [!WARNING]
> Programming ramblings ahead. If you don't care about programming then feel free skip to the [conclusion](#conclusion)
...
as a *token* and makes changes to these using macros which are expanded into more tokens.

**Typst also supports structuring this data.** There are most notably arrays and dictionaries.
...
Do you want to extract the first name of an author? You can take the full name and split it at the whitespace to get an array of two strings. Then you just have to pick the first:
```typst
#let frodo = (
    name: "Frodo Baggins",
    stud_id: "1234567",
...
This lets you repeat actions by behavior as often as you like.
...
Now since Typst treats document content as a type too we can construct content programmatically. We can take our list `autoren`, use kuerzel on each of the entries using the `map` function meaning we now

```typst
// Map transforms each entry of a list using a function and then returns a new list
#let names = #authors.map(first_name) // names is now an array ("Frodo", "Samwise")
// join "joins" a list using a chosen separator and returns Content
#names.join(" ") // This renders in the document as `Frodo Samwise`
```

### Programmatic processing of content

If you are a programmer familiar with functional programming this allows you to do a lot of cool things in your documents.
...
Of course in the age of AI this is less useful than it once was (*"Claude, generate a 10x10 matrix in LaTeX"*),
but that is of course not all that we can do.
...
And if you are now curious about what other kinds of crazy things you can do; Check out the
...
If after reading all of this you get the feeling that I dislike LaTeX, 
